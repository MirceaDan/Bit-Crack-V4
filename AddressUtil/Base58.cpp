...
uint512_t Base58::toReallyBigInt(const std::string& s)
{
	uint512_t value = 0;

	for (int i = 0; i < s.length(); i++) {
		uint512_t multiplier = 1;
		for (int j = 1; j <= i; j++)
		{
			multiplier *= 58;
		}
		value += Base58Map::myMap[s[i]] * multiplier;
	}
...
std::string Base58::toBase58(const uint512_t& x)
{
	std::string s;
